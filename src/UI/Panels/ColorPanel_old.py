...
class ColorPanel(QWidget):
    ...
    def _create_buttonBar(self, lang):
        ...
        btn_new.clicked.connect(self._new_color)
        ...
        btn_delete.clicked.connect(self._del_color)
    ...
    def _get_search_criterions(colors, lang):
    ...
    def _match_filter_with_color(self, my_filter, color):
        if my_filter["name"] not in color.Name:
            return False
        if not re.match(my_filter["company"], color.Company_name):
            return False
        if not re.match(my_filter["colorType"], color.Color_type):
            return False
